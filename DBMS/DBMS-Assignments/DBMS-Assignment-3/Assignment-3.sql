...
select order_details.cart_id ,order_details.address ,shopper.shopper_id ,shopper.shopper_name, COUNT(order_details.cart_id) as No_Of_Orders from order_details 
...
GROUP BY order_details.date_ordered;
/*
...
EXPLAIN select order_details.total_price as Revenue ,shopper.shopper_id ,shopper.shopper_name from order_details
INNER JOIN shopper on shopper.shopper_id = order_details.cart_id
WHERE order_details.date_ordered >= CURRENT_TIMESTAMP - INTERVAL 30 DAY 
...
EXPLAIN SELECT order_details.cart_id , shopper.shopper_name ,SUM(total_price) AS revenue
FROM order_details INNER JOIN shopper ON shopper.shopper_id = order_details.cart_id 
WHERE order_details.date_ordered >= CURRENT_TIMESTAMP - INTERVAL 30 DAY 
GROUP BY order_details.cart_id
ORDER BY revenue DESC
...
SELECT cart.product_id ,product.product_name,COUNT(cart.product_id) as TOP_PRODUCT from cart 
INNER JOIN product on cart.product_id = product.product_id
where product.date_added >= CURRENT_TIMESTAMP - INTERVAL 60 DAY
